...
public class ApprovePostCommandHandlerTests
{
    ...
    private readonly ApprovePostCommandHandler _commandHandler;

    public ApprovePostCommandHandlerTests()
    {
        _mocker = new AutoMocker();
        _commandHandler = _mocker.CreateInstance<ApprovePostCommandHandler>();
    }

    [Fact]
    public async Task Handle_WhenValidRequest_ShouldApprovePost()
    {
        // Arrange
        var command = ApprovePostCommandMock.GetFaker();
    ...
    [Fact]
    public async Task Handle_WhenPostNotFound_ShouldNotApprovePostAndNotify()
    {
        // Arrange
        var command = ApprovePostCommandMock.GetFaker();
